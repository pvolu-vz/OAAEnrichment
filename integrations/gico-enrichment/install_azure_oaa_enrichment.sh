#!/usr/bin/env bash
# install_azure_oaa_enrichment.sh
# One-command installer for Azure OAA Enrichment → Veza OAA integration
# Usage:
#   Interactive:       bash install_azure_oaa_enrichment.sh
#   Non-interactive:   VEZA_URL=... VEZA_API_KEY=... bash install_azure_oaa_enrichment.sh --non-interactive
set -euo pipefail

# ─────────────────────────────────────────────────────────────────────────────
# Defaults
# ─────────────────────────────────────────────────────────────────────────────
INSTALL_DIR="/opt/azure-oaa-enrichment"
...
sudo cp "${REPO_CLONE_DIR}/${SCRIPT_SUBDIR}/azure_oaa_enrichment.py" "${SCRIPTS_DIR}/"
...
if [[ -f "${ENV_FILE}" && "${OVERWRITE_ENV}" == "false" ]]; then
    ...
    if [[ "${NON_INTERACTIVE}" == "true" ]]; then
        ...
        _IDP_DOMAIN="${IDP_DOMAIN:-smurfitwestrock.com}"
        _ENTITY_TYPE="${ENTITY_TYPE:-AzureADUser}"
        _PROVIDER_NAME="${ENRICHMENT_PROVIDER_NAME:-Azure Email Enrichment}"
        _PROVIDER_ID="${ENRICHMENT_PROVIDER_ID:-}"
        _DATA_SOURCE_NAME="${ENRICHMENT_DATA_SOURCE_NAME:-Azure Email Enrichment}"
    else
        echo ""
        ...
        read -rp "  IDP domain for principal_name replacement [default: smurfitwestrock.com]: " _IDP_DOMAIN
        _IDP_DOMAIN="${_IDP_DOMAIN:-smurfitwestrock.com}"
        read -rp "  Entity type [default: AzureADUser]: " _ENTITY_TYPE
        _ENTITY_TYPE="${_ENTITY_TYPE:-AzureADUser}"
        read -rp "  Enrichment provider name [default: Azure Email Enrichment]: " _PROVIDER_NAME
        _PROVIDER_NAME="${_PROVIDER_NAME:-Azure Email Enrichment}"
        read -rp "  Enrichment provider ID (leave blank to auto-create): " _PROVIDER_ID
        read -rp "  Enrichment data source name [default: Azure Email Enrichment]: " _DATA_SOURCE_NAME
        _DATA_SOURCE_NAME="${_DATA_SOURCE_NAME:-Azure Email Enrichment}"
    fi

    sudo tee "${ENV_FILE}" > /dev/null <<EOF
# Azure OAA Enrichment — Veza credentials
# Generated by installer on $(date -u +"%Y-%m-%dT%H:%M:%SZ")
VEZA_URL=${_VEZA_URL}
VEZA_API_KEY=${_VEZA_API_KEY}
IDP_DOMAIN=${_IDP_DOMAIN}
ENTITY_TYPE=${_ENTITY_TYPE}
ENRICHMENT_PROVIDER_NAME=${_PROVIDER_NAME}
ENRICHMENT_DATA_SOURCE_NAME=${_DATA_SOURCE_NAME}
# Set ENRICHMENT_PROVIDER_ID to skip name-based lookup/creation (optional):
${_PROVIDER_ID:+ENRICHMENT_PROVIDER_ID=${_PROVIDER_ID}}
${_PROVIDER_ID:-# ENRICHMENT_PROVIDER_ID=}
EOF
...
python3 "${SCRIPT_DIR}/azure_oaa_enrichment.py" \
...
echo ""
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║   Azure OAA Enrichment — Installation Complete                ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
echo "  Install directory : ${INSTALL_DIR}"
echo "  Script            : ${SCRIPTS_DIR}/azure_oaa_enrichment.py"
...
echo "  2. Dry-run:"
echo "       ${VENV_DIR}/bin/python3 ${SCRIPTS_DIR}/azure_oaa_enrichment.py --dry-run"
echo "  3. Push enrichment:"
echo "       ${VENV_DIR}/bin/python3 ${SCRIPTS_DIR}/azure_oaa_enrichment.py"
echo "  4. Optionally schedule via cron:"
echo "       echo '0 2 * * * root ${WRAPPER} >> ${LOGS_DIR}/azure-oaa-enrichment.log 2>&1' \\"
echo "           | sudo tee /etc/cron.d/azure-oaa-enrichment"
echo ""
